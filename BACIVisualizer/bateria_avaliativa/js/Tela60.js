function carregar_dados()
{
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		...
		{
			if(!json.Paciente.Pacientes[i].hasOwnProperty("Tela60"))
			{
...
{
	json.Paciente.Pacientes[i].Tela60 = {};
	var ls = JSON.stringify(json);
	localStorage.setItem("Pacientes", ls);
}	

function carregar_paciente(json, i) {
	Object.keys(json.Paciente.Pacientes[i].Tela60).forEach(function(nome) {		
		document.getElementById(nome).checked = json.Paciente.Pacientes[i].Tela60[nome];
	});
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			...
			for(j = 0; j < allChecks.length; j++)
			{
				...
				{
					json.Paciente.Pacientes[i].Tela60[allChecks[j].id] = allChecks[j].checked;
				...
				{
					json.Paciente.Pacientes[i].Tela60[allChecks[j].name] = pontos;
				}
			}
		}
	}
...
function voltar()
{
		...
		window.location.href = "Tela59.html"
}
...
function proximo(form) {
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			if(json.Paciente.Pacientes[i].TelaAtual < 60)
				json.Paciente.Pacientes[i].TelaAtual = 60;
			var ls = JSON.stringify(json);
			localStorage.setItem('Pacientes', ls);
			salvar();
			form.action = "Tela61.html";
			break;
		}
	}
}
