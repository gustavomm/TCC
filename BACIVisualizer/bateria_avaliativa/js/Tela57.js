function carregar_dados()
{
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		...
		{
			if(!json.Paciente.Pacientes[i].hasOwnProperty("Tela57"))
			{
...
{
	json.Paciente.Pacientes[i].Tela57 = {};
	json.Paciente.Pacientes[i].Tela57.Flor = false;
	json.Paciente.Pacientes[i].Tela57.Gato = false;
	json.Paciente.Pacientes[i].Tela57.Vaso = false;
	json.Paciente.Pacientes[i].Tela57.Pincel = false;
	json.Paciente.Pacientes[i].Tela57.Coelho = false;
	json.Paciente.Pacientes[i].Tela57.Violao = false;
	json.Paciente.Pacientes[i].Tela57.Dado = false;
	json.Paciente.Pacientes[i].Tela57.Batom = false;
	json.Paciente.Pacientes[i].Tela57.Prato = false;
	json.Paciente.Pacientes[i].Tela57.Gude = false;
...
function carregar_paciente(json, i) {
	Object.keys(json.Paciente.Pacientes[i].Tela57).forEach(function(nome) {		
		document.getElementById(nome).checked = json.Paciente.Pacientes[i].Tela57[nome];
	});
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			...
			for(j = 0; j < allChecks.length; j++)
			{
				...
				{
					json.Paciente.Pacientes[i].Tela57[allChecks[j].name] = allChecks[j].checked;
				}
			}
		}
	}
...
function voltar()
{
		...
		window.location.href = "Tela56.html"
}
