function carregar_dados()
{
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		...
		{
			if(!json.Paciente.Pacientes[i].hasOwnProperty("Tela56"))
			{
...
{
	json.Paciente.Pacientes[i].Tela56 = {};
	json.Paciente.Pacientes[i].Tela56.Flor = false;
	json.Paciente.Pacientes[i].Tela56.Gato = false;
	json.Paciente.Pacientes[i].Tela56.Vaso = false;
	json.Paciente.Pacientes[i].Tela56.Pincel = false;
	json.Paciente.Pacientes[i].Tela56.Coelho = false;
	json.Paciente.Pacientes[i].Tela56.Violao = false;
	json.Paciente.Pacientes[i].Tela56.Dado = false;
	json.Paciente.Pacientes[i].Tela56.Batom = false;
	json.Paciente.Pacientes[i].Tela56.Prato = false;
	json.Paciente.Pacientes[i].Tela56.Gude = false;
...
function carregar_paciente(json, i) {
	Object.keys(json.Paciente.Pacientes[i].Tela56).forEach(function(nome) {		
		document.getElementById(nome).checked = json.Paciente.Pacientes[i].Tela56[nome];
	});
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			...
			for(j = 0; j < allChecks.length; j++)
			{
				...
				{
					json.Paciente.Pacientes[i].Tela56[allChecks[j].name] = allChecks[j].checked;
				}
			}
		}
	}
...
function voltar()
{
		...
		window.location.href = "Tela55.html"
}
...
function proximo(form) {
	...
	for(i = 0; i < json.Paciente.Pacientes.length; i++)
	{
		if(json.Paciente.Pacientes[i].Nome == sessionStorage.getItem("PacienteSelecionado"))
		{
			if(json.Paciente.Pacientes[i].TelaAtual < 57)
				json.Paciente.Pacientes[i].TelaAtual = 57;
			var ls = JSON.stringify(json);
			localStorage.setItem('Pacientes', ls);
			salvar();
			form.action = "Tela57.html";
			break;
		}
	}
}
